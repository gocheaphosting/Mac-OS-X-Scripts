...
if [ x$execute = xgo ] ; then
	...
	echo Need to specify search parameters. See usage.
	exit
...
if [ x$mid = x ] ; then
	echo You need to specify a file extension to search for.
elif [ x$ext = x ] ; then
	ext=$mid
	mid=
fi
...
#		imgno=$(echo "$imgno" | sed 's/^\(.*[IM][MV][GI]_\)\([[:digit:]]*.*_\).*$/\2/')
		imgno=$(echo "$imgno" | sed -n 's/^\(.*[IM][MV][GI]_\)\([-[:alnum:]]*\)_[[:digit:]].*$/\2/p')
