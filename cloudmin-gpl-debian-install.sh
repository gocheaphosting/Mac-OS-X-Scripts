...
apt-get -y install perl openssl libio-pty-perl libio-stty-perl libnet-ssleay-perl libwww-perl libdigest-hmac-perl libdigest-sha1-perl libxml-simple-perl libcrypt-ssleay-perl libauthen-pam-perl cron bind9 xen-tools
if [ "$?" != 0 ]; then
	echo .. install failed
	exit 1
fi
...
apt-get -y install xen-utils-3.2 || apt-get -y install xen-utils
if [ "$?" != 0 ]; then
	echo .. install failed
	exit 1
fi
apt-get -y install xen-hypervisor-3.2-1-$arch || apt-get -y install xen-hypervisor-3.2 || apt-get -y install xen-hypervisor
if [ "$?" != 0 ]; then
	echo .. install failed
	exit 1
fi
