...
apt-get -y install perl openssl libio-pty-perl libio-stty-perl libnet-ssleay-perl libwww-perl libdigest-hmac-perl libdigest-sha1-perl libxml-simple-perl libcrypt-ssleay-perl libauthen-pam-perl cron bind9 lsof parted
if [ "$?" != 0 ]; then
	echo .. install failed
	exit 1
fi
...
/usr/share/webmin/server-manager/setup-kvm-bridge.pl
brex=$?
if [ "$brex" = 0 ]; then
